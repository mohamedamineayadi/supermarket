...
bool caisses::ajouter()
{
    QSqlQuery query;
    QString ic= QString::number(ID_CAISSE);
    QString sd= QString::number(SOLDE_DEP);
    QString sf= QString::number(SOLDE_FINAL);



    query.prepare("INSERT INTO CAISSES (ID_CAISSE, SOLDE_DEP, SOLDE_FINAL) "
                        "VALUES (:ID_CAISSE, :SOLDE_DEP, :SOLDE_FINAL)");

    query.bindValue(":ID_CAISSE", ic);
    query.bindValue(":SOLDE_DEP", sd);
    query.bindValue(":SOLDE_FINAL", sf);
    return    query.exec();
}
bool caisses::ModifyCMD(int idcaisse, int sd)
{
        QSqlQuery query;
        query.prepare("UPDATE CAISSES SET SOLDE_DEP= :SOLDE_DEP WHERE ID_CAISSE=:ID_CAISSE");
        query.bindValue(":ID_CAISSE", idcaisse);
        query.bindValue(":SOLDE_DEP",sd);
        return    query.exec();
