...
bool commandes_achats::ajouter(int code){
    QSqlQuery  q1;

    q1.prepare("INSERT INTO CMDACHAT (REFCMDA, DATE_CMD,CODEFOURNISSEUR,ID_PRODUIT)"
                 "VALUES (:ref_cmd,:date_cmd,:codefournisseur,:idproduit)");
  q1.bindValue(":ref_cmd",ref_cmd);
  q1.bindValue(":date_cmd",date_cmd);
  q1.bindValue(":codefournisseur",codefournisseur);
  q1.bindValue(":idproduit",idproduit);
  q1.exec();
  return true;
}
