...
_model->setHeaderData(0, Qt::Horizontal, QObject::tr("DATE/HEURE"));
_model->setHeaderData(1, Qt::Horizontal, QObject::tr("REFERENCE"));
_model->setHeaderData(2, Qt::Horizontal, QObject::tr("PRIX"));
...
bool cmdventes::ajouter()
{
    ...
    QString DCMDV = QString(DATE_CMD);
    query.prepare("INSERT INTO CMDVENTES (REFCMDV, DATE_CMD, ID_CLIENT, ID_CAISSE, ID_PRODUIT, PRIX_CMDV) "
                        "VALUES (:REFCMDV, :DATE_CMD, '1', '1','1', :PRIX_CMDV)");

    query.bindValue(":REFCMDV", RCMDV);
    query.bindValue(":PRIX_CMDV", PCMDV);
    query.bindValue(":DATE_CMD", DCMDV);
    return    query.exec();
}
